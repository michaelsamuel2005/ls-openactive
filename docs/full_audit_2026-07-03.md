...
**M2 — Decision log: CORRECTED FINDING (2026-07-03).** The original audit stated `docs/decision-log.md` was absent — **that was wrong** (the auditor's file listing was truncated; the log exists and consolidates D-007–D-013, committed 1 July). The *remaining* real gaps: D-001–D-006 originals still not migrated (need the old `decisions.md` from the Claude project); D-014 reframe, D-015, D-016 and the 2026-07-03 correction record not yet entered (now appended); `claude-project-instructions.md` and `prompt-playbook.md` still carry pre-audit stale items. Severity downgraded Major → Moderate.

**M3 — Reproducibility/process gaps. [PARTIALLY CORRECTED + LARGELY REMEDIATED 2026-07-03]** Correction: CI *does* exist (`.github/workflows/ci.yml`, committed 1 July — ruff + all three test modules on PRs) but had never run because nothing was pushed. Remediated since: `environment.yml` pinned; all of the week's work committed and pushed; PR open (CI now exercising for the first time — an unused import was caught pre-emptively and fixed). Still open: **branch protection on `main`** (Settings → Branches: require the CI check + 1 review, block direct pushes) and marking CI as a required check; CLAUDE.md's stale environment section.
...
## 5b. Independent execution audit (sandbox, 2026-07-04) — full log: `results/panel_verification_2026-07-04.txt`

Fresh implementations (no repo imports) run against all raw data: **every published number reproduces exactly** — Open Sessions (London + national), the event harvest, all raw→features transformations (to float epsilon), the gap construction (4.4e-16), all validation statistics including Moran's I with an independently built adjacency matrix, and all 26 manifest rows (0 failures). New findings: (a) the London CSV is a proven faithful subset of the raw feed (494/494 ids, 100% price agreement); (b) the event file's 107 "duplicate" ids are cross-feed id collisions, not repeats — composite (feed,id) key needed in the next harvest; headline impact <0.06%; (c) event data spans Feb 2023–Mar 2027 — state the window in any intensity claim; (d) **bootstrap CI for ρ (5,000 resamples, seeded): [0.097, 0.705]** — corroborates Fisher [0.131, 0.696]; commit a bootstrap script before citing it.
