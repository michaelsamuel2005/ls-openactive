...
**Owner:** Fahmi Alshahabi · Consolidated 18 August 2026 · Updated 19 August 2026 · v1.4
...
> **v1.4 change:** row 10 added — Geifman & El-Yaniv 2017 attested (R-13), supporting the F-08
> empirical risk–coverage fallback under CA-01.A4.
...
## Row 10 — Geifman & El-Yaniv (2017)

| field | entry |
|---|---|
| source | Geifman, Y., & El-Yaniv, R. (2017). Selective Classification for Deep Neural Networks. In *Advances in Neural Information Processing Systems 30 (NeurIPS 2017)*, pp. 4878–4887. Curran Associates. No DOI. Version of record: NeurIPS proceedings, https://papers.nips.cc/paper/7073-selective-classification-for-deep-neural-networks |
| sections/pages read | pp. 4878–4880, problem definition, coverage and selective risk; pp. 4880–4883, Selective Guaranteed Risk (SGR) algorithm and threshold construction; pp. 4883–4886, theoretical guarantee and empirical risk–coverage evaluation; p. 4887, conclusions and practical implications. |
| claim supported | The paper defines coverage as the proportion of inputs for which the classifier chooses to make a prediction, and selective risk as the error rate measured only over those covered predictions. Varying the confidence threshold produces a risk–coverage curve, making the abstention–accuracy trade-off an empirical quantity rather than a single operating point. The SGR procedure selects a threshold that, with high probability, guarantees the true selective risk does not exceed a user-specified target while maximizing coverage under that constraint. |
| assumptions/limits | The guarantee is high-probability, not deterministic: it assumes a held-out calibration sample, valid confidence scores, and i.i.d. data drawn from the same distribution. The paper does not address clustered, nested, or design-weighted sampling, so its guarantee does not transfer directly to our CAL-RISK design. Under CA-01.A4 it therefore supports empirical risk–coverage curves as the defensible fallback when the stronger Learn-then-Test guarantee cannot be claimed. |
| decision affected | F-08 empirical risk–coverage reporting; CA-01.A4 fallback route; F-06 sizing interpretation of selective risk and coverage. This is the methodological basis for reporting empirical risk–coverage curves when clustered calibration prevents claiming the finite-sample guarantee. |
| date / reader | 2026-08-19 / Fahmi Alshahabi |
| reviewer spot-check | — pending — |

**Version-of-record note.** The NeurIPS proceedings volume I read numbers this paper pp. 4878–4887.
A separate ACM Digital Library proceedings record lists pp. 4885–4894 for the same paper. This
attestation follows the NeurIPS 2017 (*Advances in NIPS 30*) pagination throughout.

---

## Outstanding

- Reviewer spot-check pending on all ten rows — non-author task, blocked with RATIFY-19-04.
...
- Row 10: two publisher records give different paginations for the same paper. The NeurIPS volume
  pagination is followed throughout and the discrepancy is recorded rather than silently resolved.