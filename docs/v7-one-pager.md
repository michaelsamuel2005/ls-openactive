...
| **K7** | Any reference is cited without a team member having read it | It is marked `TODO-VERIFY` and **may not appear in a submitted artefact.** **STATUS: CLOSED (D-027).** Razniewski & Nutt now carries *"Reference status: primary source verified 15 July 2026"* — PVLDB vol. 4 no. 11, 2011, checked against the primary source. **The `TODO-VERIFY` may be lifted.** Formatting and its fit in the critical literature argument remain open, but the rule no longer fires. |

**One kill rule is firing: K6** — the headline figures are not reconcilable from any committed artefact (D-026). It is not fixable by harvesting more data; it needs a re-harvest with the fixed code plus a committed run log.

**K7 closed today (D-027)** — someone verified Razniewski & Nutt against the primary source.
